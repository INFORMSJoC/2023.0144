...
UE_RHO::UE_RHO(int m_rho, matrix_type D, edge_matrix_type edge_matrix, std::vector<uint> destination_count, std::vector<vertex_type> centroids, bool all_centroids, ublas_vector_type tolling_charges, int N_TOLLS) {
    this->rho = m_rho;
    m_D = D;
...
void UE_RHO::init_paths_and_graph(graph_type& g) {
    ...
    for (it1 = m_D.begin1(), itd = m_destination_count.begin(); it1 != m_D.end1(); ++it1, ++itd) {
        ...
        for (matrix_type::const_iterator2 it2 = it1.begin(); it2 != it1.end(); ++it2) {
            ...
            for (uint i = 0; i < path.n_edges(); i++) {
                ...
                g[current_edge].update_rho(g[current_edge].flow + demand, rho);
            }
...
double UE_RHO::compute_objective_twovariables(const graph_type& g, const path_it_type& M_p, const path_it_type& m_p, std::vector<edge_iterator>& M_p_edge_list, std::vector<edge_iterator>& m_p_edge_list, const double& alpha) {
    double f1 = 0.;
    double f2 = 0.;
    for (std::vector<edge_iterator>::iterator it = M_p_edge_list.begin(); it != M_p_edge_list.end(); ++it) {
        f1 += g[**it].cost_fun.integral(std::max(g[**it].flow - alpha, 0.), rho);
    }
    for (std::vector<edge_iterator>::iterator it = m_p_edge_list.begin(); it != m_p_edge_list.end(); ++it) {
        f2 += g[**it].cost_fun.integral(g[**it].flow + alpha, rho);
    }
    return f1 + f2;
}

double UE_RHO::quadratic_linesearch(const graph_type& g, const path_it_type& M_p, const path_it_type& m_p, std::vector<edge_iterator>& M_p_edge_list, std::vector<edge_iterator>& m_p_edge_list, const double& initial_step) {
...
void UE_RHO::update_flow(graph_type& g, const path_it_type& M_p, const path_it_type& m_p, std::vector<edge_iterator>& M_p_edge_list, std::vector<edge_iterator>& m_p_edge_list, const double& alpha) {
    M_p->m_path_flow -= alpha;
    m_p->m_path_flow += alpha;
    for (std::vector<edge_iterator>::iterator it = M_p_edge_list.begin(); it != M_p_edge_list.end(); ++it) {
        g[**it].decrease_update_rho(alpha, rho);
    }
    for (std::vector<edge_iterator>::iterator it = m_p_edge_list.begin(); it != m_p_edge_list.end(); ++it) {
        g[**it].increase_update_rho(alpha, rho);
    }
}
...
void UE_RHO::path_equilibration_method(graph_type& g, dest_list_type::iterator od_b, dest_list_type::iterator od_e, const bool& use_double_step, ODFrequencyUpdater& od_frequency_updater, double& sumDirectionalDeriv, int& newPaths, int& deletedPaths, const bool& requireMinPaths, const std::vector<vertex_type>& p_star, std::vector<edge_iterator>& M_p_edge_list, std::vector<edge_iterator>& m_p_edge_list) {
    for (; od_b != od_e; ++od_b) {
        int size = (*od_b).size();
...
void UE_RHO::outer_loop(graph_type& g, ublas_vector_type& link_flow_one) {
    ...
    double obj_fun = compute_objective_function(g, rho);
    ...
    while (1) {
        ...
        obj_fun = compute_objective_function(g, rho);
    ...
    for (boost::tie(ei, ee) = boost::edges(g); ei != ee; ++ei) {
        ...
        sysobj_val += g[*ei].flow * g[*ei].cost_fun.bpr_func(g[*ei].flow);
