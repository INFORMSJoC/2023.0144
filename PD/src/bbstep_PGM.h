...
public:
    ...
    double utol  = 2e-4;       // stop criterion for u
    double ftol  = 2e-4;       // stop criterion for fvalue
    double gtol  = 2e-4;       // stop criterion for gradient
...
public:
    /*constructor*/
    PGM(matrix_type D, edge_matrix_type edge_matrix, std::vector<uint> destination_count, std::vector<vertex_type> centroids, bool all_centroids, ublas_vector_type link_flow_one, ublas_vector_type m_tolling_charge_links_flow_one, int m_N_TOLLS, double m_lb, double m_ub, int m_num_UE);
    /*method*/
    void projection(ublas_vector_type& u);
    double bbstep_PGM(graph_type& g, ublas_vector_type& tolling_charges); // output: f(u^t,v^t)-V(u^t)
