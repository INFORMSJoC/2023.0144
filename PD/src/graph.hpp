...
struct edge_info {
    ...
    void decrease_update_rho(const double& alpha, const double& rho);
    void increase_update(const double& alpha);
    void increase_update_rho(const double& alpha, const double& rho);
};
...
void edge_info<cost_type>::update_rho(const double& flow, const double& rho) {
    this->flow = flow;
    this->cost_fun.update(flow, rho, this->weight, this->derivative);
}
...
template<typename cost_type>
void edge_info<cost_type>::decrease_update_rho(const double& alpha, const double& rho) {
    this->flow = std::max(0., this->flow - alpha);
    this->cost_fun.update(this->flow, rho, this->weight, this->derivative);
}
...
template<typename cost_type>
void edge_info<cost_type>::increase_update_rho(const double& alpha, const double& rho) {
    this->flow = this->flow + alpha;
    this->cost_fun.update(this->flow, rho, this->weight, this->derivative);
}
