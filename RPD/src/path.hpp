...
public:
    ...
    double compute_last_iter_cost(const graph_type& g) const;
};
...
Path<cost_type>::Path() {
    m_path_flow = 0.0;
    m_path_cost = 0.0;
    m_last_iter_path_flow = 0.0;
    m_last_iter_path_cost = 0.0;
}
...
template<typename cost_type>
double Path<cost_type>::compute_last_iter_cost(const graph_type& g) const {
    double path_cost = 0.0;
    typename edge_ptr_list_type::const_iterator it;
    for (it = m_path_edges.begin(); it != m_path_edges.end(); ++it) {
        path_cost += g[**it].last_iter_weight;
    }
    return path_cost;
}
