...
PGM::PGM(matrix_type D, edge_matrix_type edge_matrix, std::vector<uint> destination_count, std::vector<vertex_type> centroids, bool all_centroids, ublas_vector_type link_flow_one, ublas_vector_type m_tolling_charge_links_flow_one, int m_N_TOLLS, double m_lb, double m_ub, int m_num_UE, double eps) {
    m_D = D;
...
double PGM::bbstep_PGM(graph_type& g, ublas_vector_type& tolling_charges) {
    ...
    double phi = ue.outer_loop(g, tolling_charge_links_flow_star);
    double obj = std::pow(std::max(phi - this->eps, 0.), 2.);
    this->num_UE++;
    ublas_vector_type grad = 2 * std::max(phi - this->eps, 0.) * (tolling_charge_links_flow_one - tolling_charge_links_flow_star);
    ...
    for (int i = 0; i < maxit; i++) {
        ...
        while (1) {
            ...
            phi = ue.outer_loop(g, tolling_charge_links_flow_star);
            obj = std::pow(std::max(phi - this->eps, 0.), 2.);
            grad = 2 * std::max(phi - this->eps, 0.) * (tolling_charge_links_flow_one - tolling_charge_links_flow_star);
            this->num_UE++;
...
double PGM::bbstep_PGM_orig(graph_type& g2, ublas_vector_type& u) {
    ...
    double phi = ue.outer_loop(g2, tolling_charge_links_flow_two);
    double gap = phi - this->eps;
    double f = std::pow(std::max(gap, 0.), 2.);
    num_UE++;
    ublas_vector_type g = 2 * std::max(gap, 0.) * (tolling_charge_links_flow_one - tolling_charge_links_flow_two); // gradient
    ...
    for (int i = 0; i < maxit; i++) {
        ...
        while (1) {
            ...
            phi = new_ue.outer_loop(g2, tolling_charge_links_flow_two);
            gap = phi - this->eps;
            f = std::pow(std::max(gap, 0.), 2.);
            g = 2 * std::max(gap, 0.) * (tolling_charge_links_flow_one - tolling_charge_links_flow_two);
