#ifndef PATH_EQUILIBRATION_H_
#define PATH_EQUILIBRATION_H_
...
public:
    ...
    typedef std::list<Path<bpr> > path_list_type;
    ...
    int nEquilibrations;
    ublas_vector_type link_flow_one;

public:
    /*constructor*/
    UE(matrix_type D, edge_matrix_type edge_matrix, std::vector<uint> destination_count, std::vector<vertex_type> centroids, bool all_centroids, ublas_vector_type tolling_charges);
    UE(matrix_type D, edge_matrix_type edge_matrix, std::vector<uint> destination_count, std::vector<vertex_type> centroids, bool all_centroids, ublas_vector_type tolling_charges, ublas_vector_type link_flow_one);
    ...
    double outer_loop(graph_type& g); // output: V(u)
    double outer_loop(graph_type& g, ublas_vector_type& tolling_charge_links_flow_star); // output: f(u,v^t)-V(u)
};

#endif /*PATH_EQUILIBRATION_H_*/