...
#define RELATIVE_OBJ_DIFFERENCE_THRESHOLD 1e-8
...
template<typename edge_ptr_list_type>
struct path_info_type {
    uint origin;
    uint destination;
    std::size_t m_hash;
    double m_path_flow;
    double m_last_iter_path_flow;
    edge_ptr_list_type m_path_edges;

    path_info_type() :
            m_hash(), m_path_flow(0.), m_last_iter_path_flow(0.) {
    }

    path_info_type(uint m_origin, uint m_destination) :
            origin(m_origin), destination(m_destination), m_hash(), m_path_flow(0.), m_last_iter_path_flow(0.) {
    }

    inline void set_value(const std::size_t& hash, const double& path_flow, const double& last_iter_path_flow, const edge_ptr_list_type& path_edges) {
        this->m_hash = hash;
        this->m_path_flow = path_flow;
        this->m_last_iter_path_flow = last_iter_path_flow;
        this->m_path_edges = path_edges;
    }
};
...
template<typename list_it_type, typename list_it_rho_type, typename graph_type, typename path_it_type, typename path_it_rho_type, typename edge_matrix_type, typename edge_matrix_rho_type>
double get_mvp_directional_deriv(const graph_type &g, list_it_type b, list_it_rho_type b_rho, path_it_type& m_p, 
        path_it_type& M_p, path_it_rho_type m_p_rho, path_it_rho_type M_p_rho, const bool& requireMinPaths,
        const std::vector<typename graph_type::vertex_descriptor> &p_star, const edge_matrix_type& edge_matrix, const edge_matrix_rho_type& edge_matrix_rho) {

    typedef typename std::iterator_traits<list_it_type>::value_type path_list_type;
    typedef typename path_list_type::value_type path_type;
    typedef typename std::iterator_traits<list_it_rho_type>::value_type path_list_rho_type;
    typedef typename path_list_rho_type::value_type path_rho_type; // Path<bpr_rho>
    ...
    path_list_rho_type& path_list_rho(*b_rho);
    m_p_rho = path_list_rho.end();
    M_p_rho = path_list_rho.end();
    double m_v_rho = std::numeric_limits<double>::max();
    double M_v_rho = std::numeric_limits<double>::min();

    std::vector<double> path_cost_vector;

    for (path_it_type p(path_list.begin()); p != path_list.end(); ++p) {
        double cost = p->compute_cost(g);
        path_cost_vector.push_back(cost);
    ...
    int index1 = 0;
    for (path_it_rho_type p(path_list_rho.begin()); p != path_list_rho.end(); ++p) {
        double cost = path_cost_vector[index1];
    ...
    if (requireMinPaths) {
        path_type new_path(path_list.begin()->m_origin, path_list.begin()->m_destination);
        build_path(new_path, p_star, edge_matrix);
        path_rho_type new_path_rho(path_list_rho.begin()->m_origin, path_list_rho.begin()->m_destination);
        build_path(new_path_rho, p_star, edge_matrix_rho);
        double cost = new_path.compute_cost(g);
        ...
        if (std::find_if(path_list_rho.begin(), path_list_rho.end(), same_path<path_rho_type>(new_path_rho)) == path_list_rho.end()) {
            if (cost < m_v_rho) {
                new_path_directional_deriv = robust_difference(cost, M_v_rho);
                if (new_path_directional_deriv < 0.) {
                    m_p_rho = path_list_rho.insert(path_list_rho.end(), new_path_rho);
                    new_path_found = true;
                }
            }
        }
