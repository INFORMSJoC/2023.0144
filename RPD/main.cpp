...
int main(int argc, char** argv) {
    ...
    double sysobj_val;
    if (case_index > 1) {
        inner_gapval_tol = 1e-5;
    }
    else {
        inner_gapval_tol = 1e-10;
    }
    int it = 1;
    ...
    while (1) {
        ...
        // algo 3
        while (1) {
            UE ue = *(new UE(D, edge_matrix, destination_count, centroids, all_centroids, tolling_charges));
            double V_u = ue.outer_loop(graph.g);
            UE_RHO ue_rho = *(new UE_RHO(rho, epsilon, V_u, D1, edge_matrix1, destination_count1, centroids1, all_centroids1, tolling_charges, N_TOLLS));
            ...
            PGM pgm = *(new PGM(D, edge_matrix, destination_count, centroids, all_centroids, link_flow_one, tolling_charge_links_flow_one, N_TOLLS, lb, ub, num_UE, epsilon));
            ...
            this_inner_iter_phi = sysobj_val + rho * gap_val;
        ...
        std::cout << "Iteration " << it << ":" << " Step 1 of Algorithm RPD converges!" << std::endl;
